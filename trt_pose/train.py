...
if __name__ == '__main__':
    ...
    summaryWriter = SummaryWriter('runs/densenet121_320x320')   
...
#             loss.backward()
            ...
            if i % 10 == 0:
                print("epoch {} - iteration {}: average loss {:.7f}".format(epoch+1, i, running_loss/10))
                summaryWriter.add_scalar("training_loss",running_loss/10, epoch * len(train_loader) + i)
                running_loss = 0.0
        ...
        for image, cmap, paf, mask in tqdm.tqdm(iter(test_loader)):
            ...
            with torch.no_grad():
                ...
                test_running_loss += float(loss)
                if i % 10 == 0:
                    print("epoch {} - iteration {}: average loss {:.7f}".format(epoch+1, i, test_running_loss/10))
                    summaryWriter.add_scalar("testing_loss",test_running_loss/10, epoch * len(test_loader) + i)
                    test_running_loss = 0.0
